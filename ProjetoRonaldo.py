...
# Função para determinar turno por hora
def turnoDia():
    hoje = datetime.now().hour
    if hoje < 12:
        return "Manhã"
    else:
        return "Tarde"
...
def salvar():
    ...
    if nome and pagamento:
        ...
        sheet.append_row([nome, valor, pagamento, data, turnoDia(), len(sheet.get_all_records()) + 1], value_input_option="USER_ENTERED")

        # Salva no histórico local
        historico.append((nome, valor, pagamento, data, turnoDia(), len(sheet.get_all_records())))
...
#Turno
Turno = ctk.CTkLabel(app, text=f"Turno: {turnoDia()}", font=("Arial", 14))
Turno.pack(pady=5)
...
# Tabela
tabela = ttk.Treeview(frame_tabela, columns=("Nome Vendedor", "Valor", "Pagamento", "Data", "Turno", "ID"), show="headings")
...
tabela.column("Data", anchor="center")
tabela.column("Turno", anchor="center")
tabela.column("ID", anchor="center")
